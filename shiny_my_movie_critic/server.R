function(input, output, session) {
  ...
  # Changes tab, and creates a vector with chosen movie titles
  observeEvent(input$toRate, {
    updateTabsetPanel(session, 'inTabset',
                      selected = 'rate')
  })
  observeEvent(input$toResults, {
    movie_vec = c(input$movie1, input$movie2, input$movie3, input$movie4, input$movie5)
    score_vec = c(as.numeric(input$rating1)/10, as.numeric(input$rating2)/10, as.numeric(input$rating3)/10, as.numeric(input$rating4)/10, as.numeric(input$rating5)/10)
